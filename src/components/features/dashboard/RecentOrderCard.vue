  ...
  >
    <div class="p-6">
      ...
      <div v-if="!order" class="py-8 text-center">
        <span class="material-symbols-outlined text-4xl text-slate-300 dark:text-slate-600">package_2</span>
        <p class="text-slate-600 dark:text-slate-400 font-medium mt-2">{{ $t('dashboard.noRecentOrder') }}</p>
        <p class="text-slate-500 dark:text-slate-500 text-sm mt-1">{{ $t('dashboard.noRecentOrderHint') }}</p>
        <RouterLink
          to="/products"
      ...
      <template v-else>
        <div class="flex gap-6 items-center">
          <div class="w-32 h-20 bg-slate-50 dark:bg-slate-800 rounded-lg flex items-center justify-center p-2 overflow-hidden">
            <img
              v-if="order.image"
              ...
              :src="order.image"
            />
            <span v-else class="material-symbols-outlined text-slate-400 text-3xl">image</span>
          </div>
          <div class="flex-1">
            ...
            <p class="text-[#578e89] dark:text-slate-400 text-sm mt-1">
              #{{ order.orderNumber }} • {{ $t('dashboard.estArrival', { date: order.estimatedArrival }) }}
            </p>
          </div>
        </div>
        ...
        <div v-if="order.status !== 'cancelled'" class="mt-8">
          ...
          <div v-if="order.statusMessageKey || order.statusMessage" class="flex items-center gap-2 mt-3 text-slate-500 dark:text-slate-400">
